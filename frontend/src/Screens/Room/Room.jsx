...
function Room() {
  ...
  useEffect(() => {
    if (myVideoRef.current && mystream) {
      ...
      myVideoRef.current.onloadedmetadata = () => {
        myVideoRef.current
          .play()
          .catch((e) => console.log("Force play error:", e));
      };
    }
  }, [mystream]);
  ...
  useEffect(() => {
    socket.on("join-room", (data) => {
      ...
      console.log(
        `Email : ${email} has joined room: ${roomCode}. No of Participants: ${data.userCount}`,
      );
      setIsJoined(true);
    });
  ...
  useEffect(() => {
    const handleUserConnected = (data) => {
      ...
      console.log(
        `Email: ${email}, Name: ${name} has connected, SocketId - ${socket_id}`,
      );
    ...
    const callUser = async (remoteId, remoteName) => {
      ...
      call.on("stream", (remoteStream) => {
        ...
        remoteStream.getTracks().forEach((track) => {
          ...
          track.onmute = () =>
            console.log(`${track.kind} track was MUTED by the system`);
    ...
    <>
      ...
      <div className="main">
        ...
        <div className="streams">
          {mystream && (
            <div className="my-stream">
              <video autoPlay muted={true} playsInline ref={myVideoRef} />
          ...
          {remoteStream && (
            <div className="remote-stream">
              <video
                ...
                ref={remoteVideoRef}
                onPlaying={() =>
                  console.log("Remote video is officially playing!")
                }
        ...
        <div className="call-nav">
          {remoteStream && (
            <button style={{ backgroundColor: "red" }} onClick={disconnect}>
              <PhoneOff />
            </button>
          )}
          ...
          {mystream && !isJoined && (
            ...
            >
              {mystream ? "Join Meeting" : "Starting Camera..."}
            </button>
          )}
        </div>
      </div>
